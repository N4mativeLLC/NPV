...
public class AnalyticsInsertDbase {
	...
	public static void main(String[] args) {
	    ...
	    String KEY_FILE_LOCATION = args[2];//"/Users/surbhi/Documents/workspace/nVidYoWebapp/src/main/java/org/nvidyo/webapp/npv/videoJSTest.json";
        ...
        String dimsDimension_event ="ga:date,ga:pagePath,ga:eventAction,ga:eventLabel";
	...
	private static void deleteEventAnalytics(String startDate, String endDate) {
		...
		String strSQL = "{call sp_deleteEventsAnalyticsData(?,?)}";
	...
	private static void deleteAnalytics(String startDate, String endDate) {
		CallableStatement stmt = null;
		String strSQL = "{call sp_deleteAnalyticsData(?,?)}";
	...
	private static List<Campaign> geneateEvent(GaData result_event) {
		...
		if(rows!=null){
			for(int i=0;i<rows.size();i++){
				...
				event.setEventDate(record.get(0));
			    event.setEventCampaign(campaign);
			    event.setEventClient(clientName);
				event.setEventAction(record.get(2));
	    		event.setEventlabel(record.get(3));
	    		event.setTotalEvents(Integer.parseInt(record.get(4)));
	    		eventLst.add(event);
	...
	private static void insertEventAnalytics(List<Campaign> eventLst) {
		...
		CallableStatement stmt = null;
		String strSQL = "{call sp_InsertEventAnalytics(?,?,?,?,?,?)}";
